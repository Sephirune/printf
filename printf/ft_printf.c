...
static int	parse_types(char type, va_list arg)
{
	if (type == 'c')
		...
		return (ft_putunbr(va_arg(arg, int)));
	else if (type == 'u')
		return (ft_putnbr(va_arg(arg, unsigned int)));
	else if (type == 'x')
		return (ft_hexnbr(va_arg(arg, unsigned long), 0));
	else if (type == 'X')
		return (ft_hexnbr(va_arg(arg, unsigned long), 1));
	else if (type == 'p')
		return (ft_putptrnull(va_arg(arg, unsigned long)));
	else if (type == '%')
		return (ft_putchar('%'));
	return (0);
}
